...
if __name__ == '__main__':
	...
	with utilities.DeviceConnection.createTcpConnection(args) as router:
		...
		while True:
			...
			if time.time()-t0>action_time_interval and success_flag==True and beer_mode==False:
			...
			if (target_reached_flag==True and token==True and success_flag==True) or beer_mode==True:
				...
				if elapsed_time>beer_time:
					...
					# beer_pose = highlevel_movements.posetrans(startpose,translation=[0,0,0.2],rotation=[0,0,0])
					#beer_pose = highlevel_movements.posetrans(beer_pose,translation=[0,0,1],rotation=[0,0,0])
					# print(beer_pose)
					# tool_twist_duration=7
					# if time.time()-reach_beer_twist_timer<tool_twist_duration:
					# 	highlevel_movements.tool_twist_time(base,tool_twist_duration,pose_distance=[0,-0.2,0.5,0,0,0])
					# else:
					# 	base.Stop()
					new_pose=pose_trans_test.posetrans_angie(startpose,[0,0,0.1,0,0,0],"yzx") #yzx YXZ zyx
					print("startpose ",startpose," new_pose ",new_pose[0:3]+startpose[3:6])
					highlevel_movements.move_to_waypoint_linear(base, base_cyclic, new_pose[0:3]+startpose[3:6], speed=0.1)
					break
						#break
