...
speedj=5 #seconds NOTE: sadly the synchronized angular movement does not work... only when time is defined
speedl=0.20
testing=False
##
			...
			if (target_reached_flag==True and token==True and success_flag_shared.value==True) or beer_mode==True:
				...
				if elapsed_time.value>beer_time:
					...
					if time.time()-reach_beer_twist_timer<tool_twist_duration:
						highlevel_movements.tool_twist_time(base,tool_twist_duration,pose_distance=[0,-0.35,1.0,0,0,0])
					else:
						base.Stop()
						...
						highlevel_movements.angular_action_movement(base,joint_positions=[268, 240, 178, 213, 186, 56, 270],speed=speedj) #Dog Pose
