...
#picture
image2 = cv.imread("rectanglepicture.png")
imgGray2 = cv.cvtColor(image2,cv.COLOR_BGR2GRAY)
imgBlur2 = cv.GaussianBlur(imgGray2,(7,7),1)
imgCanny2 = cv.Canny(imgBlur2,50,50)
imgContour2 = image2.copy()
_, bw = cv.threshold(imgCanny2, 50, 255, cv.THRESH_BINARY | cv.THRESH_OTSU)
contours, _ = cv.findContours(bw, cv.RETR_LIST, cv.CHAIN_APPROX_NONE)
for i, c in enumerate(contours):
    area = cv.contourArea(c)
    if area < 1e2 or 1e5 < area:
        continue
    cv.drawContours(imgContour2, contours, i, (255, 0, 0), 2)
    getOrientation(c, imgContour2)
imgStack2 = stackImages(0.6,([image2,imgGray2,imgBlur2],
                            [imgCanny2,imgCanny2,imgContour2]))



#shows picture in 6 edited wais
cv.imshow("Stacked Images Picture", imgStack2)
